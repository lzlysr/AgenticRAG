...
from config import ACTIVE_DATA_DIR, ACTIVE_INDEX_DIR, RESULTS_DIR
...
def main():
    ...
    parser.add_argument("--model", default="Qwen3-4B", help="Model name in vLLM or remote API")
    ...
    parser.add_argument("--max-samples", type=int, default=982)
    parser.add_argument("--max-turns", type=int, default=7)
    parser.add_argument(
        "--qa-files-financial",
        default="qa_pairs_zh_clean.json",
        help=(
            "Comma-separated financial QA JSON files. Use an empty value to "
            "combine train_qa_pairs_zh_clean.json and qa_pairs_zh_clean.json."
        ),
    )
    parser.add_argument("--data-dir", default=None, help="Legacy directory containing qa_pairs.json")
    parser.add_argument("--index-dir", default=ACTIVE_INDEX_DIR)
    parser.add_argument("--device", default=None, help="Retrieval device (default: env RETRIEVAL_DEVICE or least-used GPU)")
    parser.add_argument("--workers", type=int, default=5)
    parser.add_argument("--temperature", type=float, default=0.7)
    args = parser.parse_args()

    device = args.device or _select_retrieval_device()
    ...
    if args.data_dir:
        ...
        qa_data = load_financial_qa_pairs(DEFAULT_FINANCIAL_QA_FILES)
    ...
    out_path = os.path.join(out_dir, f"agentic_{n}_{safe_model}-origin.json")
